...
Response<>(taskManagementService.fetchTasksByDate(request)); 
    ...
    public Response<TaskManagementDto> updateTaskPriority(
        ...
        TaskManagementDto updated = taskManagementService.updateTaskPriority(
                request.getTaskId(), request.getPriority());

        return new Response<>(updated);
    }

    @PostMapping("/comment")
    public Response<String> addComment(@RequestBody TaskCommentRequest request) {
        taskManagementService.addComment(request.getTaskId(), request.getComment(), request.getAuthor());
        return new Response<>("Comment added.");
    }

    @GetMapping("/details/{id}")
    public Response<TaskDetailsDto> getTaskDetails(@PathVariable Long id) {
        return new Response<>(taskManagementService.findTaskDetails(id));
    }
